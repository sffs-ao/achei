             ...
             <ul className="flex flex-col gap-2 w-full " >
                    {
                        MENU.map((item) => {
                            return (
                                <Link className="" key={item.name} to={item.link}><li className="hover:bg-zinc-100 px-6 py-4 rounded-sm flex gap-4 text-zinc-800  text-nowrap"><span>{item.icon} </span><span className={`${!openSideBar && "w-0 overflow-hidden"}`}>{item.name}</span></li></Link>
                            )
                        })
                    }
